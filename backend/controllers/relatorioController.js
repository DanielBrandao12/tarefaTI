...
const getViewTickets = async (req, res) => {
  try {
    const { intervalo, tipoRelatorio } = req.body;

    if (!intervalo || !tipoRelatorio) {
    ...
    switch (intervalo) {
      case "hoje":
        whereClause.data_criacao = { [Op.gte]: new Date().setHours(0, 0, 0, 0) };
        break;
      case "semana":
      ...
      case "mes":
        ...
        break;
      case "ano":
        whereClause.data_criacao = { [Op.gte]: new Date(new Date().getFullYear(), 0, 1) };
        break;
      case "todos":
        break;
      default:
    ...
    //  Buscar tickets sem a associação direta
    ...
    return res.json({ sucesso: true, dados: agrupados });
  } catch (error) {
